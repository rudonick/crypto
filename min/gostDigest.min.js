...
*/
(function(A,y){"function"===typeof define&&define.amd?define(["gostRandom","gostCipher"],y):"object"===typeof exports?module.exports=y(require("gostRandom"),require("gostCipher")):A.GostDigest=y(A.GostRandom,A.GostCipher)})(this,function(A,y){function q(a,m,f,d,k){for(var r=0;r<k;r++)f[d+r]=a[m+r]}function H(a){a=new Uint8Array(a);for(var m=new Uint8Array(a.length),f=0,d=a.length;f<d;f++)m[d-f-1]=a[f];return m.buffer}function I(a){for(var m=a.length,f=3*m+1>>2,d=new Uint8Array(f),k,r=0,l=0,s=0;s<
m;s++){k=s&3;var e=a.charCodeAt(s),e=64<e&&91>e?e-65:96<e&&123>e?e-71:47<e&&58>e?e+4:43===e?62:47===e?63:0,r=r|e<<18-6*k;if(3===k||1===m-s){for(k=0;3>k&&l<f;k++,l++)d[l]=r>>>(16>>>k&24)&255;r=0}}return d.buffer}function w(a){if(a instanceof ArrayBuffer)return a;if(a&&a.buffer&&a.buffer instanceof ArrayBuffer)return 0===a.byteOffset&&a.byteLength===a.buffer.byteLength?a.buffer:(new Uint8Array(new Uint8Array(a,a.byteOffset,a.byteLength))).buffer;throw new B("ArrayBuffer or ArrayBufferView required");
}function C(a,m){var f=this.digest===D?32:64,d=this.bitLength/8,k=w(a),r=w(m),l;k.byteLength===f?l=new Uint8Array(k):(l=new Uint8Array(f),k.byteLength>f?l.set(new Uint8Array(this.digest(k))):l.set(new Uint8Array(k)));for(var k=new Uint8Array(f+r.byteLength),d=new Uint8Array(f+d),s=0;s<f;s++)k[s]=l[s]^54,d[s]=l[s]^92;k.set(new Uint8Array(r),f);d.set(new Uint8Array(this.digest(k)),f);return this.digest(d)}function J(a,m,f){a=new Uint8Array(this.sign(a,f));m=new Uint8Array(m);if(a.length!==m.length)return!1;
f=0;for(var d=a.length;f<d;f++)if(a[f]!==m[f])return!1;return!0}function E(){var a=this.bitLength,m=(A=A||z.GostRandom)?new (A||z.GostRandom):K;if(m.getRandomValues)a=new Uint8Array(Math.ceil(a/8)),m.getRandomValues(a);else throw new F("Random generator not found");return a.buffer}function L(a,m){if(0<m%8)throw new B("Length must multiple of 8");for(var f=this.bitLength/8,d=this.digest===D?32:64,k=m/8,r=this.iterations,l=this.diversifier,s=new Uint8Array(d),e=0;e<d;e++)s[e]=l;for(var g=new Uint8Array(w(this.salt)),
b=g.length,l=d*Math.ceil(b/d),h=new Uint8Array(l),e=0;e<l;e++)h[e]=g[e%b];for(var b=new Uint8Array(w(a)),c=b.length,g=d*Math.ceil(c/d),p=new Uint8Array(g),e=0;e<g;e++)p[e]=b[e%c];b=new Uint8Array(l+g);q(h,0,b,0,l);q(p,0,b,l,g);p=Math.ceil(k/f);h=new Uint8Array(p*f);for(e=0;e<p;e++){var n=new Uint8Array(d+l+g);q(s,0,n,0,d);q(b,0,n,d,l+g);for(c=0;c<r;c++)n=new Uint8Array(this.digest(n));q(n,0,h,e*f,f);for(var x=new Uint8Array(d),c=0;c<d;c++)x[c]=n[c%f];n=(l+g)/d;for(c=0;c<n;c++)for(var t=1,v,u=d-1;0<=
u;--u)v=b[d*c+u]+x[u]+t,t=v>>>8,b[d*c+u]=v&255}f=new Uint8Array(k);q(h,0,f,0,k);return f.buffer}function M(a,m){if(0<m%8)throw new B("Length must be multiple of 8");var f=m/8,d,k=new Uint8Array(w(this.context)),r=this.bitLength/8,l=Math.ceil(f/r);d=this.label?new Uint8Array(w(this.label)):new Uint8Array([38,189,184,120]);for(var s=new Uint8Array(f),e=0;e<l;e++){var g=new Uint8Array(d.length+k.length+4);g[0]=e+1;g.set(d,1);g[d.length+1]=0;g.set(k,d.length+2);g[g.length-2]=m>>>8;g[g.length-1]=m&255;
s.set(new Uint8Array(C.call(this,a,g),0,e<l-1?r:f-e*r),e*r)}return s.buffer}function N(a,m){var f=this.diversifier||1;m*=f;if(m<this.bitLength/2||0<m%8)throw new B("Length must be more than "+this.bitLength/2+" bits and multiple of 8");var d=this.bitLength/8,k=m/8,r=this.iterations,l=new Uint8Array(w(a)),s=new Uint8Array(w(this.salt)),e=s.byteLength,g=new Uint8Array(e+4);q(s,0,g,0,e);if(137438953408<k)throw new B("Invalid parameters: Length value");for(var b=Math.ceil(k/d),s=new Uint8Array(k),h=1;h<=
b;h++){g[e]=h>>>24&255;g[e+1]=h>>>16&255;g[e+2]=h>>>8&255;g[e+3]=h&255;for(var c=new Uint8Array(C.call(this,l,g)),p=c,n=1;n<r;n++)for(var c=new Uint8Array(C.call(this,l,c)),x=0;x<d;x++)p[x]^=c[x];c=(h-1)*d;q(p,0,s,c,Math.min(d,k-c))}return 1<f?(f=k/f,d=new Uint8Array(f),q(s,k-f,d,0,f),d.buffer):s.buffer}function O(a,m){if(m>this.bitLength||0<m%8)throw new B("Length can't be more than "+this.bitLength+" bits and multiple of 8");var f=this.digest===D?32:64,d=this.bitLength/8,k=a&&0<a.byteLength?new Uint8Array(w(a)):
!1,r=k?k.length:0,l=this.iterations,s=new Uint8Array(w(this.salt)),e=s.length,g=new Uint8Array(e+r);q(s,0,g,0,e);k&&q(k,0,g,e,r);for(var b=new Uint8Array(this.digest(g)),h=new Uint8Array(f),c=new Uint8Array(f),p=new Uint8Array(f),n=0;32>n;n++)h[n]="DENEFH028.760246785.IUEFHWUIO.EF".charCodeAt(n);for(var g=new Uint8Array(2*(f+d)),x=0;x<l;x++){for(n=0;n<f;n++)c[n]=h[n]^54,p[n]=h[n]^92,h[n]=0;q(c,0,g,0,f);q(b,0,g,f,d);q(p,0,g,f+d,f);q(b,0,g,f+d+f,d);q(new Uint8Array(this.digest(g)),0,h,0,d)}for(n=0;n<
d;n++)c[n]=h[n]^54,p[n]=h[n]^92,h[n]=0;g=new Uint8Array(2*d+e+r);q(c,0,g,0,d);q(s,0,g,d,e);q(p,0,g,d+e,d);k&&q(k,0,g,d+e+d,r);b=this.digest(this.digest(g));if(m===this.bitLength)return b;f=m/8;d=new Uint8Array(f);q(b,0,d,0,f);return d.buffer}function G(a){return this.deriveBits(a,8*this.keySize)}var z=this,K=z.crypto||z.msCrypto,B=z.DataError||Error,F=z.NotSupportedError||Error,P=function(){function a(b,a){for(var c=0,h,e=0;16>e;e++)c=(b[e]&65535)+(a[e]&65535)+(c||0),h=(b[e]>>>16)+(a[e]>>>16)+(c>>>
16),b[e]=c&65535|h<<16,c=h>>>16}function m(b,c){for(var a=0;16>a;a++)b[a]=c[a]}function f(b,a){for(var c=0;16>c;c++)b[c]^=a[c]}function d(b,a){m(c,b);f(c,a);for(var h=0;8>h;h++){var d,g,l=(c[h>>2]>>((h&3)<<3)&255)<<1;d=e[l];g=e[l+1];for(var p=1;8>p;p++)l=(p<<3)+h,l=(p<<9)+((c[l>>2]>>((l&3)<<3)&255)<<1),d^=e[l],g^=e[l+1];b[h<<1]=d;b[(h<<1)+1]=g}}function k(b,c,a){m(p,b);d(p,c);m(n,p);d(p,a);for(c=0;11>c;c++)d(n,s[c]),d(p,n);d(n,s[11]);f(p,n);f(b,p);f(b,a)}var r=new Int32Array(16),l=new Int32Array(16);
l[0]=512;var s=function(b){b=new Int32Array(I(b));for(var c=Array(12),a=0;12>a;a++)c[a]=new Int32Array(b.buffer,64*a,16);return c}(""),
...
g,b,h,c=new Int32Array(16),p=new Int32Array(16),n=new Int32Array(16);return function(c){g=new Int32Array(16);b=new Int32Array(16);h=new Int32Array(16);for(var e=0;16>e;e++)256===this.bitLength&&(h[e]=16843009);c=new Uint8Array(w(c));for(var e=c.length,d=e%64,f=(e-d)/64,e=0;e<f;e++){var p=new Uint8Array(c.buffer,64*e,64),p=new Int32Array(p.buffer,p.byteOffset,16);k(h,b,p);a(b,l);a(g,p)}c=new Uint8Array(c.buffer,64*f,d);d=c.length;if(!(63<d)){e=new Int32Array(16);e[0]=d<<3;f=new Uint8Array(64);for(p=
0;p<d;p++)f[p]=c[p];f[d]=1;c=new Int32Array(f.buffer,f.byteOffset,16);e=new Int32Array(e.buffer,e.byteOffset,16);k(h,b,c);a(b,e);a(g,c);k(h,r,b);k(h,r,g)}if(256===this.bitLength)for(c=new Int32Array(8),e=0;8>e;e++)c[e]=h[8+e];else for(c=new Int32Array(16),e=0;16>e;e++)c[e]=h[e];return"SC"===this.procreator||"VN"===this.procreator?H(c.buffer):c.buffer}}(),D=function(){function a(b){for(var e=new Uint8Array(32),c=0;8>c;c++)e[4*c]=b[c],e[1+4*c]=b[8+c],e[2+4*c]=b[16+c],e[3+4*c]=b[24+c];return e}function m(b){for(var e=
new Uint8Array(8),c=0;8>c;c++)e[c]=b[c]^b[c+8];q(b,8,b,0,24);q(e,0,b,24,8);return b}function f(b){b=new Uint16Array(b.buffer,0,16);var e=b[0]^b[1]^b[2]^b[3]^b[12]^b[15];q(b,1,b,0,15);b[15]=e}function d(b,e){var c=new Uint8Array(32),d=new Uint8Array(32),g=new Uint8Array(32),k=new Uint8Array(32);q(b,e,s,0,32);q(l,0,d,0,32);q(s,0,g,0,32);for(var t=0;32>t;t++)k[t]=d[t]^g[t];var v=a(k),t=l,u=new Uint8Array(8);q(t,0,u,0,8);v=new Uint8Array(this.cipher.encrypt(v,u));q(v,0,c,0,8);for(v=1;4>v;v++){u=m(d);
for(t=0;32>t;t++)d[t]=u[t]^r[v][t];g=m(m(g));for(t=0;32>t;t++)k[t]=d[t]^g[t];var w=a(k),t=c,u=8*v,y=l,A=8*v,z=new Uint8Array(8);q(y,A,z,0,8);w=new Uint8Array(this.cipher.encrypt(w,z));q(w,0,t,u,8)}for(d=0;12>d;d++)f(c);for(d=0;32>d;d++)c[d]^=s[d];f(c);for(d=0;32>d;d++)c[d]^=l[d];for(d=0;61>d;d++)f(c);q(c,0,l,0,l.length)}function k(b){for(var d=0,c=0;c<e.length;c++)d=(e[c]&255)+(b[c]&255)+d,e[c]=d,d>>>=8}var r,l,s,e,g=new Uint8Array([0,255,0,255,0,255,0,255,255,0,255,0,255,0,255,0,0,255,255,0,255,
0,0,255,255,0,0,0,255,255,0,255]);return function(b){l=new Uint8Array(32);s=new Uint8Array(32);e=new Uint8Array(32);r=Array(4);for(var a=0;4>a;a++)r[a]=new Uint8Array(32);q(g,0,r[2],0,g.length);var c=new Uint8Array(w(b));b=c.length;for(var f=b%32,n=(b-f)/32,a=0;a<n;a++){var m=new Uint8Array(c.buffer,32*a,32);k.call(this,m);d.call(this,m,0)}0<f&&(m=new Uint8Array(c.buffer,32*n),a=new Uint8Array(32),q(m,0,a,0,f),k.call(this,a),d.call(this,a,0));a=new Uint8Array(32);b*=8;for(f=0;0<b;)a[f++]=b&255,b=
Math.floor(b/256);d.call(this,a,0);d.call(this,e,0);b=l.buffer;"SC"===this.procreator&&(b=H(b));return b}}(),Q=function(){function a(a,d,e,f,b,h){return(f^d&(e^f))+b+h+1518500249+(a<<5|a>>>27)>>>0}function m(a,d,e,f,b,h){return(d^e^f)+b+h+1859775393+(a<<5|a>>>27)>>>0}function f(a,d,e,f,b,h){return(d&e|f&(d|e))+b+h+2400959708+(a<<5|a>>>27)>>>0}function d(a,d,e,f,b,h){return(d^e^f)+b+h+3395469782+(a<<5|a>>>27)>>>0}var k,r=new Uint32Array(80);return function(l){l=new Uint8Array(w(l));var s=l.length,
e=s+9;e%64&&(e+=64-e%64);k=new Uint32Array(5);k[0]=1732584193;k[1]=4023233417;k[2]=2562383102;k[3]=271733878;k[4]=3285377520;for(var g=0;g<e;g+=64){for(var b=0;64>b;b++){var h=0,c=g+b;c<s?h=l[c]:c===s?h=128:(c=e-c-1,0<=c&&4>c&&(h=s<<3>>>8*c&255));r[b>>2]=0===b%4?h<<24:r[b>>2]|h<<8*(3-b%4)}for(b=16;80>b;b++)h=r[b-3]^r[b-8]^r[b-14]^r[b-16],r[b]=h<<1|h>>>31;for(var b=k,h=r,c=b[0],p=b[1],n=b[2],q=b[3],t=b[4],v=a,u=0;80>u;u+=5)20===u?v=m:40===u?v=f:60===u&&(v=d),t=v(c,p,n,q,t,h[u]),p=(p<<30|p>>>2)>>>0,
q=v(t,c,p,n,q,h[u+1]),c=(c<<30|c>>>2)>>>0,n=v(q,t,c,p,n,h[u+2]),t=(t<<30|t>>>2)>>>0,p=v(n,q,t,c,p,h[u+3]),q=(q<<30|q>>>2)>>>0,c=v(p,n,q,t,c,h[u+4]),n=(n<<30|n>>>2)>>>0;b[0]+=c;b[1]+=p;b[2]+=n;b[3]+=q;b[4]+=t}for(b=0;5>b;b++)l=k[b],k[b]=(l&255)<<24|(l&65280)<<8|l>>8&65280|l>>24&255;return k.buffer}}();return function(a){a=a||{};this.name=(a.name||"GOST R 34.10")+"-"+(a.version||2012)%100+(1<(a.version||2012)?"-"+(a.length||256):"")+("HASH"!==(a.mode||"HASH")?"-"+a.mode:"")+(a.procreator?"/"+a.procreator:
"")+("string"===typeof a.sBox?"/"+a.sBox:"");this.procreator=a.procreator;this.bitLength=a.length||256;switch(a.version||2012){case 1:this.digest=Q;this.bitLength=160;break;case 1994:this.digest=D;this.sBox=(a.sBox||("SC"===a.procreator?"D-SC":"D-A")).toUpperCase();y||(y=z.GostCipher);if(!y)throw new F("Object GostCipher not found");this.cipher=new y({name:"GOST 28147",block:"ECB",sBox:this.sBox,procreator:this.procreator});break;case 2012:this.digest=P;break;default:throw new F("Algorithm version "+
a.version+" not supported");}this.keySize=a.keySize||(2>=a.version?this.bitLength/8:32);switch(a.mode||"HASH"){case "HASH":break;case "HMAC":this.sign=C;this.verify=J;this.generateKey=E;break;case "KDF":this.deriveKey=G;this.deriveBits=M;this.label=a.label;this.context=a.context;break;case "PBKDF2":this.deriveKey=G;this.deriveBits=N;this.generateKey=E;this.salt=a.salt;this.iterations=a.iterations||2E3;this.diversifier=a.diversifier||1;break;case "PFXKDF":this.deriveKey=G;this.deriveBits=L;this.generateKey=
E;this.salt=a.salt;this.iterations=a.iterations||2E3;this.diversifier=a.diversifier||1;break;case "CPKDF":this.deriveKey=G;this.deriveBits=O;this.generateKey=E;this.salt=a.salt;this.iterations=a.iterations||2E3;break;default:throw new F("Algorithm mode "+a.mode+" not supported");}}});
