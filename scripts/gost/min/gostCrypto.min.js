...
d=z+d[0]+A+"."+d[1],e=document.querySelector('script[src="'+d+'"]');e||(e=document.createElement("script"),e.setAttribute("src",d),(document.head||document.getElementsByTagName("head")[0]).appendChild(e))}}function t(a,c,d){return new Promise(function(e,b){try{if(w){var D=++J;v.push({id:D,resolve:e,reject:b});w.postMessage({id:D,algorithm:a,method:c,args:d})}else f.gostEngine?e(f.gostEngine.execute(a,c,d)):b(new B("Module gostEngine not found"))}catch(m){b(m)}})}function r(a){try{a()}catch(c){}}function g(){}
var f=this,k=f.crypto||f.msCrypto,u=f.SyntaxError||Error,F=f.DataError||Error,p=f.NotSupportedError||Error,B=f.OperationError||Error,H=f.InvalidAccessError||Error,G=f.InvalidAccessError||Error;f.Promise||(f.Promise=function(){function a(a){return a&&null===a.oncomplete&&null===a.onerror?new c(function(c,b){a.oncomplete=function(){c(a.result)};a.onerror=function(){b(new B(a.toString()))}}):a}function c(d){function e(a){try{a()}catch(b){}}var b="pending",f,m=[],h=[];try{d(function(a){"pending"===b&&
(b="fulfilled",f=a,m.forEach(e))},function(a){"pending"===b&&(b="rejected",f=a,h.forEach(e))})}catch(k){"pending"===b&&(b="rejected",f=k,h.forEach(e))}this.then=function(d,e){return new c(function(c,k){function q(){var b;try{b=d?d(f):f}catch(e){k(e);return}(b=a(b))&&b.then&&b.then.call?b.then(c,k):c(b)}function g(){var b;try{b=e?e(f):f}catch(d){k(d);return}(b=a(b))&&b.then&&b.then.call?b.then(c,k):k(b)}"fulfilled"===b?q():"rejected"===b?g():(m.push(q),h.push(g))})};this["catch"]=function(a){return this.then(void 0,
a)}}c.all=function(a){return new c(function(c,b){function f(a){q++;return function(b){h[a]=b;q--;0===q&&c(h)}}function k(a){0<q&&b(a);q=0}for(var h=[],q=0,g=0,p=a.length;g<p;g++){var l=a[g];l.then&&l.then.call?l.then(f(g),k):h[g]=l}0===q&&c(h)})};return c}());var z="",A="";"undefined"!==typeof document&&function(){for(var a=/^(.*)gostCrypto(.*)\.js$/i,c=document.querySelectorAll("script"),d=0,e=c.length;d<e;d++){var b=c[d].getAttribute("src");if(b=a.exec(b))z=b[1],A=b[2]}}();var w,v=[],J=0;if(!f.importScripts&&
!f.gostEngine)try{w=new Worker(z+"gostEngine"+A+".js"),w.onmessage=function(a){for(var c=a.data.id,d=0,e=v.length;d<e&&v[d].id!==c;d++);d<e&&(c=v[d],v.splice(d,1),a.data.error?c.reject(new B(a.data.error)):c.resolve(a.data.result))},w.onerror=function(a){for(var c=0,d=v.length;c<d;c++)v[c].reject(a.error);v=[]}}catch(K){w=!1}f.importScripts||(f.importScripts=y);w||f.gostEngine||y("gostEngine.js");g.prototype.encrypt=function(a,c,d){return(new Promise(r)).then(function(){if(l(a))return k.subtle.encrypt(a,
