...
d){return(new Promise(t)).then(function(){if(q(a))return k.subtle.sign(a,c,d);a=g(a,"sign");return m(a,"sign",[n("sign",a,c),d]).then(function(c){"SC"===a.procreator&&"SIGN"===a.mode&&(c=u.asn1.GostSignature.encode(c));return c})})};l.prototype.verify=function(a,c,d,e){return(new Promise(t)).then(function(){if(q(a))return k.subtle.verify(a,c,d,e);a=g(a,"verify");if("SC"===a.procreator&&"SIGN"===a.mode){var b=u.asn1.GostSignature.decode(d);d={r:b.r,s:b.s}}return m(a,"verify",[n("verify",a,c),d,e])})};
l.prototype.digest=function(a,c){return(new Promise(t)).then(function(){if(q(a))return k.subtle.digest(a,c);a=g(a,"digest");return m(a,"digest",[c])})};l.prototype.generateKey=function(a,c,d){return(new Promise(t)).then(function(){if(q(a))return k.subtle.generateKey(a,c,d);var e=a.privateKey,b=a.publicKey;a=g(a,"generateKey");e=e?g(e,"generateKey"):a;b=b?g(b,"generateKey"):a;return m(a,"generateKey",[]).then(function(h){return h.publicKey&&h.privateKey?K(b,e,c,d,h.publicKey,h.privateKey):A(a,c,d,
h)})})};l.prototype.deriveKey=function(a,c,d,e,b){return(new Promise(t)).then(function(){if(q(a))return k.subtle.deriveKey(a,c,d,e,b);a=g(a,"deriveKey");d=g(d,"generateKey");a.keySize=d.keySize;a["public"]&&(a["public"].algorithm=g(a["public"].algorithm),a["public"]=n("deriveKey",a,a["public"]));return m(a,"deriveKey",[n("deriveKey",a,c)]).then(function(a){return A(d,e,b,a)})})};l.prototype.deriveBits=function(a,c,d){return(new Promise(t)).then(function(){if(q(a))return k.subtle.deriveBits(a,c,d);
a=g(a,"deriveBits");a["public"]&&(a["public"]=n("deriveBits",a,a["public"]));return m(a,"deriveBits",[n("deriveBits",a,c),d])})};l.prototype.importKey=function(a,c,d,e,b){var h;return(new Promise(t)).then(function(){if(q(d))return k.subtle.importKey(a,c,d,e,b);if("raw"===a){d=g(d,"importKey");if(b&&b.indexOf){var f=d.name.toUpperCase().replace(/[\.\s]/g,"");0<=f.indexOf("3410")&&0<=b.indexOf("sign")?h="private":0<=f.indexOf("3410")&&0<=b.indexOf("verify")&&(h="public")}return c}var r;if("pkcs8"===
a)r=u.asn1.GostPrivateKeyInfo.decode(c).object;else if("spki"===a)r=u.asn1.GostSubjectPublicKeyInfo.decode(c).object;else throw new s("Key format not supported");d=g(r.algorithm,"importKey");h=r.type;!1!==e&&(e=e||r.extractable);if(b)for(f=0;f<b.length;f++){if(0>r.usages.indexOf(b[f]))throw B("Key usage not valid for this key");}else b=r.usages;r=r.buffer;var p=d.keySize,l=r.byteLength;if("public"===h||p===l)return r;if(0<l%p)throw new B("Invalid key size");d.mode="MASK";d.procreator="VN";for(var n=
[],f=p;f<l;f+=p)n.push(function(a){return function(b){return m(d,"unwrapKey",[a,b]).then(function(a){var b=n.pop();if(b)return b(a);delete d.mode;return a})}}(new Uint8Array(r,f,p)));return n.pop()(new Uint8Array(r,0,p))}).then(function(a){return A(d,e,b,a,h)})};l.prototype.exportKey=function(a,c){return(new Promise(t)).then(function(){if(c&&q(c.algorithm))return k.subtle.exportKey(a,c);if(!c.extractable)throw new G("Key not extractable");var d=n(null,null,c);if("raw"===a)return d;if("pkcs8"===a&&
c.algorithm&&c.algorithm.id){if("VN"===c.algorithm.procreator){var e=c.algorithm,b;e.mode="MASK";return m(e,"generateKey").then(function(a){b=a;return m(e,"wrapKey",[b,c.buffer])}).then(function(a){delete e.mode;var c=new Uint8Array(a.byteLength+b.byteLength);c.set(new Uint8Array(a,0,a.byteLength));c.set(new Uint8Array(b,0,b.byteLength),a.byteLength);a=c.buffer;a.enclosed=!0;return u.asn1.GostPrivateKeyInfo.encode({algorithm:e,buffer:a})})}return u.asn1.GostPrivateKeyInfo.encode(c)}if("spki"===a&&
c.algorithm&&c.algorithm.id)return u.asn1.GostSubjectPublicKeyInfo.encode(c);throw new s("Key format not supported");})};l.prototype.wrapKey=function(a,c,d,e){return(new Promise(t)).then(function(){if(q(e))return k.subtle.wrapKey(a,c,d,e);e=g(e,"wrapKey");var b=n(null,null,c);"SC"===e.procreator&&"private"===c.type&&(b=H(b));return m(e,"wrapKey",[n("wrapKey",e,d),b]).then(function(b){if("raw"===a)return b;throw new s("Key format not supported");})})};l.prototype.unwrapKey=function(a,c,d,e,b,f,l){return(new Promise(t)).then(function(){if(q(e))return k.subtle.unwrapKey(a,
c,d,e,b,f,l);e=g(e,"unwrapKey");b=g(b,"importKey");if("raw"!==a)throw new s("Key format not supported");return m(e,"unwrapKey",[n("unwrapKey",e,d),c]).then(function(a){var c;if(b&&b.name){var d=b.name.toUpperCase().replace(/[\.\s]/g,"");0<=d.indexOf("3410")&&0<=l.indexOf("sign")?c="private":0<=d.indexOf("3410")&&0<=l.indexOf("verify")&&(c="public")}"SC"===e.procreator&&"private"===c&&(a=H(a));return A(b,f,l,a,c)})})};u.subtle=new l;u.getRandomValues=function(a){var c=(y=y||f.GostRandom)?new y:k;if(c.getRandomValues)c.getRandomValues(a);
